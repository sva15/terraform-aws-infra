...
resource "aws_lambda_function" "functions" {
  ...
  filename         = var.use_local_source && local.actual_bucket_name == "" ? "${var.lambda_code_local_path}/${each.value}.zip" : null
  source_code_hash = var.use_local_source && local.actual_bucket_name == "" ? filebase64sha256("${var.lambda_code_local_path}/${each.value}.zip") : null
  ...
  s3_object_version = var.use_local_source && local.actual_bucket_name != "" ? aws_s3_object.lambda_code["${each.value}.zip"].version_id : null
